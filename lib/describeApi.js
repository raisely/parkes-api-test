...
function describeRoute(server, route, prefix) {
	...
	describe(route.name, () => {
		...
		beforeAll(async () => {
			...
			if (executedRoute.expect || executedRoute._expect){
				// detect if it's a raw Sequelize object
				const isRaw = isRawSequalize(executedRoute.expect || executeRoute._expect);
				if (isRaw) throw new Error('Raw Sequelize object detected as expect. Aborting');
			}
		});
