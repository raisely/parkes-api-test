...
/*	beforeAll((done) => {
	...
	try {
		routes.forEach((route) => {
			const s = app();
			describeRoute(s, route, currentPrefix);
		});
...
function describeRoute(server, route, prefix) {
	...
	describe(route.name, () => {
		...
		/* something is wrong with the chai/mocha hooks <- this is a temporary fix */
		if (route.expect || route._expect) {
			it(`returns ${route.status} with expected body`, () => new Promise((resolve, reject) => {
				...
				try{
					...
					console.log(e); // log
					chai.assert.fail(0, 1, e); // force assertion since chai is refusing to awknowledge
				}
				resolve();
			}));
		} else {
			it(`returns ${route.status}`, (done) => {
				checkStatus(executedRoute.response, executedRoute.status);
				done();
			});
		}
